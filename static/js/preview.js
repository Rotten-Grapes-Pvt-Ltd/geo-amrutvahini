    ...
    if (typeof url != "string") {
      ...
      reader.onload = function (e) {
        ...
        try {
          ...
          SHPParser.load(
            URL.createObjectURL(
              new Blob([zip.file(shpString).asArrayBuffer()])
            ),
            shpLoader,
            returnData
          );
          DBFParser.load(
            URL.createObjectURL(
              new Blob([zip.file(dbfString).asArrayBuffer()])
            ),
            encoding,
            dbfLoader,
            returnData
          ...
          Swal.fire({
            position: "center",
            icon: "error",
            title: "Unsupported shapefile! Please try again",
      ...
      JSZipUtils.getBinaryContent(url, function (err, data) {
        ...
        SHPParser.load(
          URL.createObjectURL(new Blob([zip.file(shpString).asArrayBuffer()])),
          shpLoader,
          returnData
        );
        DBFParser.load(
          URL.createObjectURL(new Blob([zip.file(dbfString).asArrayBuffer()])),
          encoding,
          dbfLoader,
          returnData
        );
  ...
  for (var i = 0; i < shpRecords.length; i++) {
    ...
    // point : 1 , polyline : 3 , polygon : 5, multipoint : 8
      switch (shpRecords[i].shape.type) {
        ...
        case 8:
          geometry.type =
            shpRecords[i].shape.type == 3 ? "LineString" : "MultiPoint";
          geometry.coordinates = [];
          for (
            var j = 0;
            j < shpRecords[i].shape.content.points.length;
            j += 2
          ) {
            var reprj = TransCoord(
        ...
        case 5:
          ...
          for (
            var pts = 0;
            pts < shpRecords[i].shape.content.parts.length;
            pts++
          ) {
            ...
            for (
              var j = partsIndex * 2;
              j <
              (shpRecords[i].shape.content.parts[pts + 1] * 2 ||
                shpRecords[i].shape.content.points.length);
              j += 2
            ) {
